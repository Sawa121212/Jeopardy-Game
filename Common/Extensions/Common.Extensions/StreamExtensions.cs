...
namespace Common.Extensions
{
    ...
    public static class StreamExtensions
    {
            ...
            CancellationTokenSource tokenSource = null)
        {
            ...
            {
                int count = offset / buffer.Length;
                for (int i = 0; i <= count; i++)
                {
                    int bytesTotal;
                    int bytesDesired = i != count
                        ? buffer.Length
        ...
        public static byte[] ReadBytes(this Stream stream, int offset, int length)
        {
            ...
            for (int bytesRead = 0; bytesRead != -1 && bytesTotal < buffer.Length; bytesTotal += bytesRead)
            ...
            byte[] buffer = null)
        {
            ...
            {
                tokenSource.Token.ThrowIfCancellationRequested();
                int count = length <= ms.Length + buffer.Length ? (int)(length % buffer.Length) : buffer.Length;
            ...
            byte[] buffer = null)
        {
            ...
            {
                int count = length <= outStream.Length + buffer.Length 
        ...
        public static int GetNewlineCharCount(this Stream stream, Encoding encoding = null)
        {
            encoding ??= Encoding.UTF8;
            int charCount;                                      // количество считанных символов
            int crLfCount = 0;                                  // количество переводов строк
