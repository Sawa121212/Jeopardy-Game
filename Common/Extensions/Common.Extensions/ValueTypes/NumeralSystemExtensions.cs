...
namespace Common.Extensions.ValueTypes
{
    ...
    public static class NumeralSystemExtensions
    {
        ...
        {
            int i = (index + alphabetOffset) % Digits.Length;
            return Digits[i];
        }

        internal static int GetIndex(int index, int alphabetOffset)
        {
            int i = (index + Digits.Length - alphabetOffset) % Digits.Length;
            return i;
        }
        ...
        public static string ToBase58(this ulong number, uint alphabetOffset = 0)
        {
            ...
            while (number != 0)
            {
                int remainder = (int)(number % (ulong)Digits.Length);
        ...
        public static ulong FromBase58(this string value, uint alphabetOffset = 0)
        {
            ...
            for (int i = value.Length - 1; i >= 0; i--)
            {
                ...
                if (!DigitDictionary.TryGetValue(c, out int digit) || digit >= Digits.Length)
                    throw new FormatException(CommonPhrases.Exception_InvalidCharInParam.Format(nameof(value), c));

                int digitIndex = GetIndex(digit, (int)alphabetOffset);
        ...
        {
            if (uint.TryParse(hexString, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out uint value))
            {
                return value;
            }
        ...
        public static byte[] HexToBytes(this string str, bool isSkipWhiteSpace = false)
        {
            ...
            if (HexToBytes(str, out byte[]? bytes, isSkipWhiteSpace))
                return bytes;
