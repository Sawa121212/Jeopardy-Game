...
namespace Common.Core.IO
{
    ...
    public static class PathExtensions
    {
        ...
        /// </summary>
        public static bool IsValidPath(string pathString) => !pathString.IsNullOrEmpty() && pathString.IndexOfAny(Path.GetInvalidPathChars()) == -1;

        /// <summary>
        /// Возвращает признак того, что имя файла состоит из валидных символов.
        /// </summary>
        public static bool IsValidFileName(string fileName) => !fileName.IsNullOrEmpty() && fileName.IndexOfAny(Path.GetInvalidFileNameChars()) == -1;
        ...
        {
            return Components.SystemInfo.OsPlatformEx.IsUnix ? '/' : '\\';
        }
        ...
        {
            return Normalize(
                pathStrings.Aggregate(string.Empty, (current, pathString) => AddLastPathSeparator(current) + RemoveFirstPathSeparator(pathString)),
                isAddLastSeparator);
        }
        ...
        public static string CorrectPathSeparator(string path, bool isAddLastSeparator = false) =>
            path.Replace('\\', GetPathSeparatorChar()).Replace('/', GetPathSeparatorChar()).Func(isAddLastSeparator, AddLastPathSeparator);
        ...
        public static string Normalize(string pathString, bool isAddLastSeparator = false)
        {
            ...
            char separator = GetPathSeparatorChar();
            string[]? arr = pathString.Split(separator);
            bool isSeparatorEndOfLine = arr[arr.Length - 1].IsNullOrWhiteSpace();
            IEnumerable<string>? newArr = arr.RemoveEmptyStr();
            string? newString = string.Join(separator.ToString(), newArr);
        ...
        public static string AddLastPathSeparator(string pathString)
        {
            if (string.IsNullOrEmpty(pathString))
                return string.Empty;
            return string.Concat(pathString, !IsLastPathSeparator(pathString) ? GetPathSeparatorString() : string.Empty);
        }
        ...
        public static string AddFirstPathSeparator(string pathString)
        {
            ...
            if ((pathString.Length > 3 && pathString[1] == ':' && pathString[2] == GetPathSeparatorChar()) ||
                (Components.SystemInfo.OsPlatformEx.IsUnix && pathString[0] == GetPathSeparatorChar()))
                return pathString;
            return string.Concat(!IsFirstPathSeparator(pathString) ? GetPathSeparatorString() : string.Empty, pathString);
        }
